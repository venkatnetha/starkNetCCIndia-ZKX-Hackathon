// The large part of the code in influenced from Dolven lab contracts.
// It has everything regarding the user proposal and signing and voting on the proposal.
...
from Libraries.Approver import Approver
...
const NAME = 6506391038712693579;  // zkx-hack
...
func constructor{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    Approver.initializer(firstSignerAddress, secondSignerAddress, initialApprover);
    return ();
}
...
func getProposalState{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    with_attr error_message("Governance::getProposalState INVALID_PROPOSAL_ID") {
...
func createProposal{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    with_attr error_message("Governance::createProposal INVALID_PROPOSAL_TYPE") {
        assert is_proposal_valid = TRUE;
    }
    with_attr error_message("Governance::createProposal INCONSISTENT_PARAMS_LENGTH") {
    ...
    with_attr error_message("Governance::createProposal EXECUTOR_NOT_AUTHORIZED") {
...
func cancelProposal{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    with_attr error_message("Governance::cancelProposal ONLY_BEFORE_EXECUTED") {
...
func queueProposal{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    Approver.onlyApprover();
    let state: felt = getProposalState(_proposalId);
    with_attr error_message("Governance::queueProposal INVALID_STATE_FOR_QUEUE") {
...
func executeProposal{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    Approver.onlyApprover();
    let state: felt = getProposalState(_proposalId);
    with_attr error_message("Governance::queueProposal ONLY_QUEUED_PROPOSALS") {
        assert state = QUEUED;
    }
...
func _submitVote{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    with_attr error_message("Governance::_submitVote VOTING_CLOSED") {
        assert state = ACTIVE;
    }
    let proposalDetails: Proposal = proposals.read(proposalId);
    let user_vote: Vote = userVotesForProposal.read(voter, proposalId);

    with_attr error_message("Governance::_submitVote VOTE_ALREADY_SUBMITTED") {
...
@external
func setExecutor{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    executor: felt
) {
    Approver.onlyApprover();
    timeLocker.write(executor);
